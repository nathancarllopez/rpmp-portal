...
export default function calculateOrderInfo(
  orders: Order[]
): { 
  ...
  for (const order of orders) {
    ...
    // Extract order info
    // To do: Check if we can get rid of tracking the product variation. Also update in parseOrders and types if we do get rid of product variation
    ...
    if (issue || !weight) {
      orderErrors.push({
        error: null,
        message: issue ?? `Could not extract weight from this size string: ${size}`,
        order
      });
      continue;
    }
    ...
    if (ingredients[protein]) {
      ingredients[protein][flavor] = weight * quantity + (ingredients[protein][flavor] ?? 0)
    } else {
  ...
  stats.thankYouBags = Object.values(orderCountByName).reduce((count, orderCount) => {
...
function getContainerSize(itemName: string): { size: string | null, weight: number | null, issue: string | null } {
  ...
  if (match.includes('lb')) {
    const weightInOz = 16 * parseFloat(match.replace('lbs', '').replace('lb', ''));
    return {
      size: "bulk",
      weight: weightInOz,
      issue: null
