...
export interface ProteinsWithFlavors {
  name: string;
  label: string;
...
async function getProteinsWithFlavors() {
  ...
  const proteinsWithFlavors: ProteinsWithFlavors[] = data.sort((rowA, rowB) => {
    return (rowA.protein_label ?? "MISSING").localeCompare(rowB.protein_label ?? "MISSING");
  }).map((row) => {
    ...
    return {
      name: row.protein_name ?? "MISSING",
      label: row.protein_label ?? "MISSING",
      flavors: flavorNames.map((flavor, idx) => ({
        value: flavor,
