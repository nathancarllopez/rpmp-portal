...
export default function CreateModal({
  ...
  const roleData: { label: string; value: string }[] =
    status === "success"
      ? data.map(({ name, label }) => ({ label, value: name }))
      : [];
  const roleExplanations: Record<string, string> = {};
  (data ?? []).forEach((row) => (roleExplanations[row.name] = row.explanation));

  const { profile } = useAuth();
  const createUserMutation = useCreateUserMutation(profile?.userId);
