import { Profile, CamelCase, Database, SettingsRow, ContainerSize, Order } from "./shared";
...
export type InsertBackstockRow = Database["public"]["Tables"]["backstock_proteins"]["Insert"];
...
export type AllBackstockRow = {
    [K in keyof SupaAllBackstockRow as CamelCase<K & string>]: SupaAllBackstockRow[K];
};
...
export type OrderHeaderRow = {
    [K in keyof SupaOrderHeaderRow as CamelCase<K & string>]: SupaOrderHeaderRow[K];
};
type SupaProteinWithFlavors = Database["public"]["Views"]["proteins_with_flavors"]["Row"];
export type ProteinWithFlavors = {
    [K in keyof SupaProteinWithFlavors as CamelCase<K & string>]: SupaProteinWithFlavors[K];
};
...
type SupaVeggieCarbInfoRow = Database["public"]["Tables"]["veggie_carb_info"]["Row"];
export type VeggieCarbInfoRow = {
    [K in keyof SupaVeggieCarbInfoRow as CamelCase<K & string>]: K extends "amounts" ? {
...
type SupaTimecardHistoryRow = Database["public"]["Tables"]["timecards_history"]["Row"];
export type TimecardHistoryRow = {
    [K in keyof SupaTimecardHistoryRow as CamelCase<K & string>]: K extends "data" ? TimecardValues[] : SupaTimecardHistoryRow[K];
};
export type InsertTimecardHistoryRow = Database["public"]["Tables"]["timecards_history"]["Insert"];
type SupaOrderHistoryRow = Database["public"]["Tables"]["order_history"]["Row"];
export type OrderHistoryRow = {
    [K in keyof SupaOrderHistoryRow as CamelCase<K & string>]: K extends "data" ? OrderReportInfo : SupaOrderHistoryRow[K];
};
